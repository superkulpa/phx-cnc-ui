...
bool CheckSize(CString _path, int _minSize, ifstream& _f){
  _f.open( _path.c_str());
  if ( _f.fail()){
    _f.close();
    return false;
...
{
    string res_str = str;
    size_t eIndx = res_str.find_last_not_of(" \t\r\n\a\b\f\v");//\0
    if (eIndx != string::npos) res_str.erase(eIndx + 1);
    size_t sIndx = res_str.find_first_not_of(" \t\r\n\a\b\f\v");//\0
    if (sIndx != string::npos) res_str.erase(0, sIndx);
    return res_str;
}

void Copy(const CString& _from, const CString& _to){
  try{
...
bool CIniFile::ReadIniFile(){
  if((ReadFile(path) == false) ){
    printf("Warning: ini file:'%s' is corrupt, try to restore from BACKUP\n", path.c_str());
    //файл попорчен, читаем из backup
    Copy(backup_path, path);
    if((ReadFile(path) == false) ){
      //везде файлы попорчены, востановить из архива
      printf("Warning: ini file:'%s' is corrupt, try to restore from ARCHIVE\n", backup_path.c_str());
      Copy(archive_path, path);
...
bool CIniFile::WriteIniFile(){
  if((WriteFile(path) == false)){
    printf("Warning: ini file:'%s' is corrupt after writing, try to replace from BACKUP\n", path.c_str());
    //файл попорчен, читаем из backup
    Copy(backup_path, path);
    if((ReadFile(path) == false)){
      //везде файлы попорчены, востановить из архива
      printf("Warning: ini file:'%s' is corrupt after writing, try to replace from ARCHIVE\n", backup_path.c_str());
      Copy(archive_path, path);
      Copy(archive_path, backup_path);
      return ReadFile(path);
    };
  }else{
    //запись успешна, обновить backup
...
bool CIniFile::WriteFile(const CString&  _path)
{
  ...
  if (1) { //вписать CRC
    valueID = FindValue(0, "CRC");
    if (valueID == unsigned(noID)) {
      //нет CRC в начале файла, добавить
      char* tmp = strdup(path.c_str());
      if(keys.size() == 0)
        AddValue("First", "CRC", basename(tmp));
      else
        SetValue(*(names.begin()), "CRC", basename(tmp), true);
      free(tmp);
    }
    valueID = FindValue(keys.size() - 1, "CRC");
    if (valueID == unsigned(noID)) {
      //нет CRC в конце файла, добавить
      char* tmp = strdup(path.c_str());
      if(keys.size() <= 1)
        AddValue("Last", "CRC", basename(tmp));
      else
        SetValue(*(names.rbegin()), "CRC", basename(tmp), true);
      free(tmp);
    }
  }
...
long CIniFile::FindKey( const CString& keyname) const
{
...
long CIniFile::FindValue( unsigned const keyID, const CString& valuename) const
{
...
unsigned CIniFile::AddKeyName( const CString& keyname)
{
  names.resize( names.size() + 1, trim(keyname));
...
unsigned CIniFile::NumValues( const CString& keyname)
{
...
bool CIniFile::SetValue( unsigned const keyID, unsigned const valueID, const CString& value)
{
  if ( keyID < keys.size() && valueID < keys[keyID].names.size())
...
bool CIniFile::SetValue( const CString& keyname, const CString& valuename, const CString& value, bool const create)
{
  long keyID = FindKey( keyname);
  if ( keyID == noID) {
...
bool CIniFile::AddValue(const CString& keyname, const CString& valuename, const CString& value){
  long keyID = FindKey( trim(keyname));
  if ( keyID == noID)    return false;

  keys[keyID].names.resize( keys[keyID].names.size() + 1, trim(valuename));
  keys[keyID].values.resize( keys[keyID].values.size() + 1, value);
  return true;
};

bool CIniFile::SetValueI( const CString& keyname, const CString& valuename, int const value, bool const create)
{
...
bool CIniFile::SetValueF( const CString& keyname, const CString& valuename, double const value, bool const create)
{
  char svalue[MAX_VALUEDATA];
...
bool CIniFile::SetValueV( const CString& keyname, const CString& valuename, char *format, ...)
{
  va_list args;
...
void CIniFile::CreateSection( const CString& keyname, const CString& valuename, const CString& value){
  ...
  //создаем CRC
  char* tmp = strdup(path.c_str());
  SetValue("CRC", "CRC", basename(tmp),true);
  free(tmp);
}


void CIniFile::CreateValue( const CString& keyname, const CString& valuename, const CString& value){
  SetValue(keyname, valuename, value, true);
  SetValue(keyname, "descr", keyname, true);
  //записываем
//  WriteIniFile();
}
...
int CIniFile::GetValueI(const CString& keyname, const CString& valuename, int const defValue)
{
  char svalue[MAX_VALUEDATA];
...
double CIniFile::GetValueF(const CString& keyname, const CString& valuename, double const defValue)
{
  char svalue[MAX_VALUEDATA];
...
void CIniFile::HeaderComment( const CString& comment)
{
...
unsigned CIniFile::NumKeyComments( const CString& keyname) const
{
  long keyID = FindKey( keyname);
  if ( keyID == noID)
    return 0;
...
bool CIniFile::KeyComment( unsigned const keyID, const CString& comment)
{
  if ( keyID < keys.size()) {
...
bool CIniFile::KeyComment( const CString& keyname, const CString& comment)
{
  long keyID = FindKey( keyname);
  if ( keyID == noID)
    return false;
...
bool CIniFile::DeleteKeyComment( const CString& keyname, unsigned const commentID)
{
  long keyID = FindKey( keyname);
  if ( keyID == noID)
    return false;
...
bool CIniFile::DeleteKeyComments( const CString& keyname)
{
  long keyID = FindKey( keyname);
  if ( keyID == noID)
    return false;
