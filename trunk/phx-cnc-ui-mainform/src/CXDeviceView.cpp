...
CXDeviceView::CXDeviceView(const QString& aDeviceName, QWidget*) :
    AXBaseWindow()
{
  setupUi(this);
...
CXDeviceView::load()
{
  ...
  for (int i = 0; i < mChannelCount; i++)
  {
    ...
    if (mChannelsTable->item(i, 1) == NULL)
      mChannelsTable->setItem(i, 1, new QTableWidgetItem);
...
CXDeviceView::loadDevices(int aGroupNumber, const QList<QPushButton*>& aButtonsList)
{
  ...
  if (!devices.isEmpty())
  {
    ...
    for (int i = 0; i < devicesList.count(); i++)
    {
      ...
      if (aButtonsList.count() > 5)
        connect(aButtonsList.at(5), SIGNAL(clicked()), curDeviceView, SLOT(editChannel()));
      if (aButtonsList.count() > 4)
        connect(aButtonsList.at(4), SIGNAL(clicked()), curDeviceView, SLOT(editDevice()));
      if (aButtonsList.count() > 3)
      {
        connect(signalMapper, SIGNAL(mapped(const QString&)), curDeviceView,
        SLOT(onSendCommand(const QString &)));
...
bool
CXDeviceView::eventFilter(QObject* watched, QEvent* e)
{
  if (watched == mChannelsTable)
  {
    ...
    case QEvent::KeyRelease:
      ...
      QList<int> incKeys;
      incKeys << Qt::Key_Q << Qt::Key_W << Qt::Key_E << Qt::Key_R << Qt::Key_T << Qt::Key_Y
          << Qt::Key_U << Qt::Key_I << Qt::Key_O << Qt::Key_P;
      QList<int> invKeys;
      invKeys << Qt::Key_A << Qt::Key_S << Qt::Key_D << Qt::Key_F << Qt::Key_G << Qt::Key_H
          << Qt::Key_J << Qt::Key_K << Qt::Key_L << Qt::Key_Semicolon;
      QList<int> decKeys;
      decKeys << Qt::Key_Z << Qt::Key_X << Qt::Key_C << Qt::Key_V << Qt::Key_B << Qt::Key_N
...
void
CXDeviceView::onFocusChange(QWidget*, QWidget*)
{
  if (QApplication::activeWindow() != NULL
      && QApplication::activeWindow()->metaObject()->className() == QString("CXDeviceView"))
  {
...
void
CXDeviceView::setDescription(int aIndex, const QString& aDescription)
{
...
void
CXDeviceView::editDevice()
{
  if (this != mLastView)
    return;
  if (mDeviceEdit == NULL)
    mDeviceEdit = new CXDeviceEdit;
...
CXDeviceView::editChannel()
{
  ...
  if (mChannelEdit == NULL)
  {
    ...
    for (int i = 0; i < keysNum; ++i)
    {
      ...
      for (int j = 0; j < valuesNum; j++)
      {
        ...
        if (QString::fromStdString(channelsFile.GetValue(keyName, valueName)).split(",").count()
            < 3)
          continue;
  ...
  if (mChannelEdit->exec())
  {
    ...
    value = value.arg(mDeviceName).arg(index).arg(newChannelData.mModify).arg(
        newChannelData.mDescription);
    std::string v = QString(value.toUtf8()).toStdString();

    if (!curChannelData.mSectionName.isEmpty())
    {
      if (curChannelData.mSectionName != newChannelData.mSectionName
          || curChannelData.mValueName != newChannelData.mValueName)
      {
        channelsFile.DeleteValue(curChannelData.mSectionName.toStdString(),
            curChannelData.mValueName.toStdString());
        channelsFile.AddValue(newChannelData.mSectionName.toStdString(),
            newChannelData.mValueName.toStdString(), v);
      }
      else
        channelsFile.SetValue(newChannelData.mSectionName.toStdString(),
            newChannelData.mValueName.toStdString(), v);
    }
    else
      channelsFile.AddValue(newChannelData.mSectionName.toStdString(),
...
void
CXDeviceView::onSendCommand(const QString& aCommand)
{
  if (this != mLastView)
    return;
