...
void
CXParamData::open(const QString& aFileName)
{
...
void
CXParamData::close(bool aIsWrite)
{
  if (aIsWrite)
    gIniFile.WriteIniFile();
  gIniFile.Erase();
}

QMap<QString, QString>
CXParamData::getKeys(const QString& aType)
{
  ...
  for (int i = 0; i < count; i++)
    {
      ...
      res.insert(QString::fromStdString(valueName),
          QString::fromStdString(gIniFile.GetValue(group, valueName)));
    }
...
QMap<QString, QString>
CXParamData::getValues(const QString& aType)
{
...
QList<SXDataXml>
CXParamData::getCaptions(const QString& aFileName, const QString& aType)
{
  QList<SXDataXml> res;

  QXmlQuery xmlQuery(QXmlQuery::XQuery10);
  xmlQuery.setQuery(
      QString("doc('%1')/Settings/ui-TechParams/%2").arg(aFileName).arg(aType));
          ...
          if (regVisible.cap(1).isEmpty() || regVisible.cap(1) != "0")
            {
              ...
              res.append(
                  SXDataXml(regExp.cap(1), regDescr.cap(1),
                      regColumn.cap(1).toInt(), regReadOnly.cap(1).toInt(),
                      regDelta.cap(1).toDouble()));
            }
...
void
CXParamData::deleteKeys(const QStringList& aKeys)
{
...
void
CXParamData::setKeysArray(const QString& aGroup, const QStringList& aKeys)
{
...
QStringList
CXParamData::getKeysArray(const QString& aGroup)
{
  std::string group = (aGroup + "/Keys").toStdString();

  return QString::fromStdString(gIniFile.GetValue(group, "Keys")).split(",",
