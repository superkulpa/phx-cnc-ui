...
CXOperDirectionView::paintEvent(QPaintEvent*)
{
  ...
  case E_Fingers:
    ...
    if (mCurrentFinger >= 0)
      painter.drawPath(mDrawist.at(mCurrentFinger));
    /*
    ...
    if (mCurrentFinger >= 0)
      painter.drawPath(mFingersList.at(mCurrentFinger));

    for (int i = 0; i < mFingersArrowList.count(); i++)
    {
      if (i >= mCurrentFinger * 3 && i < (mCurrentFinger + 1) * 3)
        painter.fillPath(mFingersArrowList.at(i), QColor(0, 0, 0, 100));
    }
...
CXOperDirectionView::mouseReleaseEvent(QMouseEvent*)
{
  ...
  else
  {
    ...
    if (mType == E_Circle)
      ...
      setDirection(OperDirectionView::E_Stop, E_NoVelocity);
...
CXOperDirectionView::createFingersType()
{
  ...
  mFingerPath.addEllipse(center, FINGER_RADIUS, FINGER_RADIUS*3);
//  mFingerPath.addRect(0, 0, 20, 7);
  ...
  mDirectionList << OperDirectionView::E_TopRight << OperDirectionView::E_Right
      << OperDirectionView::E_BottomRight <<
      OperDirectionView::E_BottomLeft << OperDirectionView::E_Left << OperDirectionView::E_TopLeft
      <<
      OperDirectionView::E_BottomRight << OperDirectionView::E_Bottom
      << OperDirectionView::E_BottomLeft <<
  ...
  for (int r = 0; r < 3; r++)
  {
    ...
    for (int i = 0; i < 3; i++)
    {
      if (i == 0)
      {
        ...
        transform = transform.translate(x + ((r == 1 ? 0 : inclWidth) + cellSize.width()) / 2.0,
            center.y() - cellSize.height() * (1.0 - r));
        if (r != 1)
          transform = transform.rotate(r == 0 ? -35.0 : 35.0);
      ...
      if (r == 1)
      {
        ...
        {
          drawPath.moveTo(
              QPointF(FINGER_RADIUS * qCos(startAngle), FINGER_RADIUS * qSin(startAngle)) + center);
        ...
        if (i == 0)
        {
          ...
          drawPath.moveTo(QPointF(x, y));
          drawPath.lineTo(
              QPointF(FINGER_RADIUS * qCos(-startAngle), FINGER_RADIUS * qSin(-startAngle))
                  + center);
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    for (int c = 0; c < directionListCount; c++)
      mDirectionPathList << transform.map(mDirectionPathList.at(c));
    for (int c = 0; c < drawistCount; c++)
      mDrawist << transform.map(mDrawist.at(c));
    for (int c = 0; c < arrowCount; c++)
      mFingersArrowList << transform.map(mFingersArrowList.at(c));
  }
}

OperDirectionView::eMoveDirection
CXOperDirectionView::getDirection(int index)
{
  return mDirectionList.at(index / 3);
}

eVelocity
CXOperDirectionView::getVelocity(int index)
{
...
CXOperDirectionView::updateDirection(const QPointF& aPos, bool aIsMousePress)
{
  ...
  if (mType == E_Fingers)
  {
    if (mCurrentFinger >= 0)
    {
      ...
      if (mCurrentDirection >= 0 && mDirectionPathList.at(mCurrentDirection).contains(aPos)
          && mCurrentDirection >= mCurrentFinger * 9
          && mCurrentDirection < (mCurrentFinger + 1) * 9)
        return;

      for (int i = 0; i < mDirectionPathList.count(); i++)
      {
        if (mDirectionPathList.at(i).contains(aPos) && i >= mCurrentFinger * 9
            && i < (mCurrentFinger + 1) * 9)
        {
          mCurrentDirection = i;
...
CXOperDirectionView::updatePaintDirection()
{
  ...
  if (mCurrentFinger >= 0 && mCurrentDirection >= 0 && getVelocity(mCurrentDirection) == mVelocity
      && getDirection(mCurrentDirection) == mDirection)
    return;
