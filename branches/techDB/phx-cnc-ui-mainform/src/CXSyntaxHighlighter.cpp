...
CXSyntaxHighlighter::CXSyntaxHighlighter(QTextDocument* parent) :
...
void
CXSyntaxHighlighter::highlightBlock(const QString& text)
{
  ...
  highlight(text, "(M|T)(-?\\d+\\.?\\d*)", mMTextFormat);
  highlight(text, "M\"[^\"]*\"", mMTextFormat);
  highlight(text, "(G)(-?\\d+\\.?\\d*)", mGTextFormat);
...
void
CXSyntaxHighlighter::highlight(const QString& aText, const QString& aPattern, const QTextCharFormat& aFormat, int aCapNumber)
{
...
CXIniSyntaxHighlighter::CXIniSyntaxHighlighter(QTextDocument* parent) :
    QSyntaxHighlighter(parent)
{
...
void
CXIniSyntaxHighlighter::highlightBlock(const QString& text)
{
...
void
CXIniSyntaxHighlighter::highlight(const QString& aText, const QString& aPattern, const QTextCharFormat& aFormat)
{
  QRegExp expression(aPattern);
  expression.setCaseSensitivity(Qt::CaseInsensitive);
...
CXXmlSyntaxHighlighter::CXXmlSyntaxHighlighter(QTextDocument* parent) :
    QSyntaxHighlighter(parent)
{
...
void
CXXmlSyntaxHighlighter::highlightBlock(const QString& aText)
{
  ...
  if (prevBlockState <= E_NoneHighlight)
    startIndex = tagBodyStart.indexIn(aText);
  ...
  if (prevBlockState == E_AttributeBody1 || prevBlockState == E_AttributeBody2)
  {
    ...
    if (prevBlockState == E_AttributeBody1)
      c = '"';
    else
      c = '\'';
  ...
  while (startIndex >= 0)
  {
    ...
    if (endIndex < 0)
    {
      ...
      endIndex += tagBodyEnd.matchedLength();
    ...
    while (tempIndex >= 0)
    {
      ...
      if (tempIndex >= 0)
      {
        ...
        if (tempIndex >= 0
            && (attrName.indexIn(aText, startIndex) < 0
                || tempIndex <= attrName.indexIn(aText, startIndex)))
        {
          ...
          if (c == '"' || c == '\'')
          {
            ...
            if (tempIndex < 0)
            {
              ...
              if (c == '"')
                setCurrentBlockState(E_AttributeBody1);
              else
                setCurrentBlockState(E_AttributeBody2);
            }
