...
main(int argc, char *argv[])
{
  ...
  if (QFile::exists(resFileName))
    QFile::remove(resFileName);
  ...
  if (sourceFile.open(QIODevice::ReadOnly))
  {
    ...
    QString queryText(
        "INSERT INTO tbl_plasma_params VALUES(NULL, :cons_angle, :plasma_source, :metal_type, :min_thickness, :max_thickness, :gases, :amperage, :out_description,\
    ...
    while (!stream.atEnd())
    {
      QString tmpStr = stream.readLine();
      sourceData = tmpStr.split(";");
      if (sourceData.count() >= 36)
      {
        ...
        if (!metalTypes.contains(temp))
          metalTypes.insert(temp, metalTypes.count() + 1);
        ...
        //query.bindValue(":min_thickness", newThick);
        query.bindValue(":max_thickness", newThick);

        prevThick = newThick;

        temp = sourceData.at(5) + "/" + sourceData.at(6);
        if (!gasesTypes.contains(temp))
          gasesTypes.insert(temp, gasesTypes.count() + 1);
        ...
        query.bindValue(":out_burning_z_distance",
            sourceData.at(23).toDouble() / 100.0
                * sourceData.at(22).toDouble());
        query.bindValue(":out_cutting_z_distance",
            sourceData.at(20).toDouble());
        ...
        for (int i = 26; i <= 32; i++)
        {
          ...
          if (!imagesMap.contains(temp))
            imagesMap.insert(temp, imagesMap.count() + 1);
        ...
        query.bindValue(":out_plasma_preflow_rate",
            sourceData.at(9).toDouble());
        query.bindValue(":out_shield_preflow_rate",
            sourceData.at(10).toDouble());
        query.bindValue(":out_plasma_cutflow_rate",
            sourceData.at(11).toDouble());
        query.bindValue(":out_shield_cutflow_rate",
            sourceData.at(12).toDouble());
    ...
    queryText =
        "INSERT INTO tbl_plasma_sources VALUES(1, :name, :description, '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '');";
    query.prepare(queryText);
    ...
    queryText =
        "INSERT INTO tbl_plasma_metal_types VALUES(:id, :name, :description);";
    ...
    queryText =
        "INSERT INTO tbl_gas_types VALUES(:id, :gases, :mixed, :description);";
    ...
    for (mapIter = imagesMap.begin(); mapIter != imagesMap.end(); mapIter++)
    {
      ...
      if (!files.isEmpty())
      {
        ...
        if (temp.split(QRegExp("( |\\.)")).count() >= 2)
        {
          ...
          if (!imagesTypes.contains(temp2))
            imagesTypes.insert(temp2, imagesTypes.count() + 1);
      ...
      if (imageType != -1)
        query.bindValue(":type", imageType);
      else
    ...
    queryText =
        "INSERT INTO tbl_torch_parts_types VALUES(:id, :name, :position, :manuf, :description);";
    ...
    queryText =
        "INSERT INTO tbl_parts_groups VALUES(:id, :description, :part_1, :part_2, :part_3, :part_4, :part_5, :part_6, :part_7, :part_8, :part_9, :part_10);";

    for (mapIter = imagesGroups.begin(); mapIter != imagesGroups.end();
        mapIter++)
    {
      query.prepare(queryText);
