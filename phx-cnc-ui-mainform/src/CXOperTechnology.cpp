...
AXBaseWindow(), zMoveType(false), warmDlg(NULL)
{
  ...
  //  mOperVelocity->setTexkeyts(QList<QStsvrring>() << "^" << trUtf8("стоп\n-\nавто") << "v");
...

  {
    ...
    for(int i = 0; i < countOfSupp; i++){
      ...
      mSVRZ.push_back(QPair<QLabel*, QLabel*>( new QLabel(frSuppList), new QLabel(frSuppList)));
      mSVRZ.last().first->setAlignment(Qt::AlignCenter);
      mSVRZ.last().second->setAlignment(Qt::AlignCenter);
      mSVRZ.last().first->setText("V:125");

      layoutZ->addWidget(mSVRZ.last().first);
      layoutZ->addWidget(mSVRZ.last().second);

      horizontalLayout_4->addStretch();
      horizontalLayout_4->addItem(layoutZ);
      horizontalLayout_4->addStretch();
    ...
    do{
      ...
      if (aCommand ==  (Commands::MSG_STATE_SVR_VOLTAGE))
      {
        ...
        for (int i = 0; i < list.count(); i++)
        {
          ...
          {
            mSVRZ.at(index).first->setText(QString("V:")
                + currentValue.mid(currentValue.indexOf("=") + 1).left(3));
          }
        }
        break;
      }
      if (aCommand ==  (Commands::MSG_STATE_Z_STATUS))
      {
        QStringList currentValue = aValue.split('=');
        int index = currentValue.at(0).toInt();
        if (index >= 0 && index < mSVRZ.count())
        {
          mSVRZ.at(index).second->setText(currentValue.at(1));
        }
        break;
      }
