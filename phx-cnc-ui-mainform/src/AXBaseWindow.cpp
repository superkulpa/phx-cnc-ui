...
void
AXBaseWindow::registerManager()
{
  if (mManager != NULL)
    mManager->addWindow(this);
}

void
AXBaseWindow::setNewGeometry(const QRect& aNewGeometry)
{
  if (aNewGeometry.size() == size())
    update();
...
int
AXBaseWindow::groupNumber()
{
  return mGroupNumber;
}

void
AXBaseWindow::setFreeze(bool aIsFreeze)
{
  mIsFreeze = aIsFreeze;
  mResizeType = E_None;
  update();
...
bool
AXBaseWindow::isFreeze()
{
  return mIsFreeze;
}

void
AXBaseWindow::mousePress(QMouseEvent* e)
{
  if (mIsFreeze)
    return;
...
bool
AXBaseWindow::mouseMove(QMouseEvent* e)
{
  if (mIsFreeze)
    return false;
  ...
  if (mIsButtonPress && e->buttons() == Qt::LeftButton)
  {
    ...
    if (mResizeType != E_None)
    {
      if (mResizeType == E_Left || mResizeType == E_TopLeft || mResizeType == E_BottomLeft)
      {
        ...
        if (geom.width() < resSize)
          geom.setLeft(geom.right() - resSize);
      }

      if (mResizeType == E_Top || mResizeType == E_TopLeft || mResizeType == E_TopRight)
      {
        geom.setTop(e->globalPos().y());
        if (geom.height() < resSize)
          geom.setTop(geom.bottom() - resSize);
      }
      ...
      if (mResizeType == E_Bottom || mResizeType == E_BottomLeft || mResizeType == E_BottomRight)
      {
        ...
        if (geom.height() < resSize)
          geom.setHeight(resSize);
      }
    }
    else
      geom.moveTo(mPos + e->globalPos() - mPressPos);
...
void
AXBaseWindow::mouseRelease(QMouseEvent*)
{
  if (mIsFreeze)
    return;
...
void
AXBaseWindow::mousePressEvent(QMouseEvent* e)
{
...
void
AXBaseWindow::mouseMoveEvent(QMouseEvent* e)
{
...
void
AXBaseWindow::mouseReleaseEvent(QMouseEvent* e)
{
...
void
AXBaseWindow::paintEvent(QPaintEvent* e)
{
  ...
  if (mResizeType == E_Top || mResizeType == E_TopLeft || mResizeType == E_TopRight)
    paintColor = selectColor;
  else
    paintColor = acceptColor;
...
void
AXBaseWindow::leaveEvent(QEvent* e)
{
...
void
AXBaseWindow::closeEvent(QCloseEvent* e)
{
...
eSideType
AXBaseWindow::getResizeType(const QRect& aRect, const QPoint& aPos)
{
  ...
  if (area.contains(aPos - aRect.topLeft()))
    return E_TopLeft;
  if (area.contains(aPos - aRect.topRight()))
    return E_TopRight;
  if (area.contains(aPos - aRect.bottomLeft()))
    return E_BottomLeft;
  if (area.contains(aPos - aRect.bottomRight()))
    return E_BottomRight;

  if (aPos.y() - aRect.top() <= RESIZE_BORDER)
    return E_Top;
  if (aPos.x() - aRect.left() <= RESIZE_BORDER)
    return E_Left;
  if (aRect.right() - aPos.x() <= RESIZE_BORDER)
    return E_Right;
  if (aRect.bottom() - aPos.y() <= RESIZE_BORDER)
    return E_Bottom;
