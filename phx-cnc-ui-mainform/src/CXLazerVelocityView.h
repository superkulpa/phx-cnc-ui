...
public:
  ...
  void
  setMode(eVelocityMode aMode);

  //! Функция установки текста описания делений.
  void
  setTexts(const QList<QString>& aTexts);
...
signals:
  ...
  void
  velocityChanged(eVelocity aVelocity);
...
private:
   ...
   */
  void
  updateVelocity(const QPointF& aPos);

  //! Функция установки текущего значения скорости.
  void
  setCurrentVelocity(eVelocity aVelocity);
