...
CXChannelEdit::CXChannelEdit(QWidget* parent) : QDialog(parent)
{
	setupUi(this);
	...
	connect(mSectionsList, SIGNAL(currentItemChanged(QListWidgetItem*, QListWidgetItem*)), this, SLOT(onSectionChange()));
	connect(mNamesList, SIGNAL(currentItemChanged(QListWidgetItem*, QListWidgetItem*)), this, SLOT(onNameChange()));
}
...
void CXChannelEdit::setData(const SXChannelData& aData)
{
	mChannelData = aData;
	...
	if (!mChannelData.mSectionName.isEmpty())
	{
		...
		mNamesList->setCurrentRow(mSectionsMap.values(mChannelData.mSectionName).indexOf(mChannelData.mValueName));
	}
	else
	{
...
const SXChannelData& CXChannelEdit::getData()
{
	return mChannelData;
}

void CXChannelEdit::setSectionData(const QMap <QString, QString>& aSectionsMap)
{
...
void CXChannelEdit::onSectionChange()
{
	mNamesList->clear();
	if (mSectionsList->currentItem() == NULL) return;
...
void CXChannelEdit::onNameChange()
{
	if (mNamesList->currentRow() >= 0)
	{
		...
		QString value = QString::fromStdString(channelsFile.GetValue(sectionName.toStdString(), valueName.toStdString()));
