...
CXDeviceEdit::CXDeviceEdit(QWidget* parent) :
    QDialog(parent)
{
...
void
CXDeviceEdit::setData(const QString& aData)
{
  ...
  for (int i = 0; i < mEditList.count(); i++)
  {
    ...
    if (curData.mLineEdit->text().isEmpty())
    {
      QMessageBox::critical(this, trUtf8("Ошибка"),
          trUtf8("Введите данные поля %1.").arg(curData.mLabel->text()));
